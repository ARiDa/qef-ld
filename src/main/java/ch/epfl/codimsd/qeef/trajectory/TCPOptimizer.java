...
/**
 * 
 * The TCPOptimizer optimizes a query execution plan (QEP). It performs the following operations :
 ...
 */
public class TCPOptimizer extends Optimizer {
	 ...
	 * Log4j logger.
	 */
	private static Logger logger = Logger.getLogger(TCPOptimizer.class.getName());
	 ...
	 * Default constructor.
	 */
	public TCPOptimizer(Request request) {
	...
	private void updateQEPInitial(QEP qepInitial) {
		...
		if (idDistributedTopCut > 1) {
			...
			_id[0]=idB2I;
                        ...
                        if(topCutNode.getOpName().equals("Eddy")){
                                int[] tempIDs = {3,4};
                                topCutNode.setProducerIDs(tempIDs);
                        }
                        else
		...
		String qepString = QEPFactory.generateString(qepInitial, "Initial");
                ...
                System.out.println("qeplocal = " + qepString);
...
//				+ "_" + + System.currentTimeMillis() + ".xml", qepString);
	...
	private void updateQEPDistributedLocal(int numberOfLocalOperators, int numberOfDistributedOperators) {
		...
		for (int i = qepDistributedLocal.getOperatorList().size(); i > 0; i--) {
			...
			if (tempOpNode.getOpName().equalsIgnoreCase(Constants.Block2InstanceOperator) == true) {
				...
				//int[] tempIDs = {idDistributedTopCut};
                                int[] tempIDs = {2};
	...
	private void updateQEPDistributedRemote() {
		...
		for (int i = numberOfNewOperators; i > 0; i--) {
			
			OpNode tempOpNode = (OpNode) temporaryHashtable.get(i+"");
                        
                        int[] tempIDs = tempOpNode.getProducerIDs();
                        if(!tempOpNode.getOpName().equals("Scan"))
                        {
                            if(tempOpNode.getOpName().equals("SpacialHashJoin")){
                                tempIDs[0] = idDistributedTopCutRemote+(numberOfNewOperators-i+7);
                                tempIDs[1] = idDistributedTopCutRemote+(numberOfNewOperators-i);
                            }
                            else if(tempOpNode.getOpName().equals("TemporalHashJoin")){
                                tempIDs[0] = idDistributedTopCutRemote+(numberOfNewOperators-i);
                                tempIDs[1] = idDistributedTopCutRemote+(numberOfNewOperators-i-3);
                            }
                            /*else if(tempOpNode.getOpName().equals("Unfold")){
                                tempIDs[0] = 6;
                            }
                            else if(tempOpNode.getOpName().equals("Eddy")){
                                tempIDs[0] = idDistributedTopCutRemote+(numberOfNewOperators-i-8);
                                tempIDs[1] = idDistributedTopCutRemote+(numberOfNewOperators-i);
                                ...
                                for (int k = 0; k < tempIDs.length; k++) {
                                        tempIDs[k] = idDistributedTopCutRemote+(numberOfNewOperators-i);
                                }

                        }
                        tempOpNode.setProducerIDs(tempIDs);
			
			qepDistributedRemote.getOperatorList().put((idDistributedTopCutRemote+(numberOfNewOperators-i)+1)+"", tempOpNode);
		}
		...
		for (int i= 1; i <= (infoNodes.getNumberOfNodes()); i++) {
			...
			String qepString = QEPFactory.generateString(qepRemote, "Remote");
                        ...
                        System.out.println("qeplremote = " + qepString);
	...
	private void loadQEPTemplates() throws QEPInitializationException {
		...
		try {
			...
			String qepDistributedLocalName = QEPFactory.
				getQEP(Constants.REQUEST_TYPE_TCP, 1);
		
			String qepDistributedRemoteName = QEPFactory.
				getQEP(Constants.REQUEST_TYPE_TCP, 2);
	...
	private void callG2N() throws CatalogException {
		...
		//	String ratesRelation = "";
			...
			while (rset.next() == true) {
				...
				fnode = new FragmentInfo(idNode, rates[k], Constants.msgProcessingTime, Constants.netTransmissionTime , 0);
//				ratesRelation += ratesRelation + rates[k] + ";";
			...
			if (bl.containsKey(Constants.QEP_SCAN_NUMBER_TUPLES)) {
				...
				String sourceName = QEPFactory.getScanSource(qepInitial).toUpperCase();
				initialNumberOfTuples = (Integer) catalogManager.getSingleObject("DS_TABLE",
						"numberoftuples","name='"+sourceName+"'");
			}
	...
	private int[] findDistributedOperators(int environment) throws CatalogException {
		...
		while (firstParallelOperatorFound == false && i+1 <= operators.size()) {
			i++;
		...
		while (lastParallelOperatorFound == false && i+1 <= operators.size()) {
			i++;
			OpNode opNode = (OpNode) operators.get(i+"");
